  ...
  useEffect(() => {
    ...
    const runProcess = async () => {
      ...
      try {
        ...
        setStatus("scraping")
        setProgress(20)
        addLog(`INIT_CONNECTION: ${new URL(url).hostname}`)
        addLog(`MODE: ${category.toUpperCase()}`)
        
        // Single call to backend that handles everything
        console.log("Calling backend with:", { url, category });
        const result = await generateContentAction(url, category)
        
        if (!mounted) return
        
        addLog(`BACKEND_SUCCESS: ID ${result.article_id}`)
        setProgress(60)
        setStatus("synthesizing")

        // Construct mock objects to satisfy the UI since backend doesn't return full metadata
        const scraped: ScrapedContent = {
            title: "Processed Content",
            content: "Content processed server-side",
            markdown: "",
            url: url,
            wordCount: 0,
            estimatedReadTime: 0
        ...
        const concept: Concept = {
            title: `Generated ${category.charAt(0).toUpperCase() + category.slice(1)}`,
            description: "Content generated by AI based on the source article.",
            prompt: "Prompt generated server-side (not returned in API)",
            visual: result.media_url,
            tone: category.toUpperCase()
        }
        
        setConcepts([concept])
        addLog(`MEDIA_RECEIVED`)
